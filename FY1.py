...
def scan_stocks(selected_boards, selected_industries, **params):
    ...
    progress_func = params['progress_func']
    
    debug_mode = params.get('debug_mode', False)
   
    ma_period = params.get('ma_period', 30)
    ma_deviation_lower = params.get('ma_deviation_lower', -20)
    ma_deviation_upper = params.get('ma_deviation_upper', 20)
   
    all_ma_periods = [5, 10, 20, 30, 60, 120, 250]
   
    # 去掉市值限制参数
    min_turn = params.get('min_turn', 0.5)
    max_turn = params.get('max_turn', 50)
    enable_limit_strength = params.get('enable_limit_strength', False)
    limit_days_max = params.get('limit_days_max', 30)
    enable_huangniu = params.get('enable_huangniu', False)
    enable_my_pattern = params.get('enable_my_pattern', True)
    enable_buy_points = params.get('enable_buy_points', True)
    high_pos_threshold = params.get('high_pos_threshold', 0.98)
   
    log_func("=" * 60)
    log_func("黄牛选股器 v2.0 (无市值限制版)")
    log_func("=" * 60)
    log_func(f"数据源: {DATA_SOURCE}")
    log_func(f"均线: {ma_period}日，偏离度 [{ma_deviation_lower}%, {ma_deviation_upper}%]")
    log_func(f"换手率: {min_turn}-{max_turn}%")
    log_func(f"形态识别: {'启用' if enable_my_pattern else '禁用'}")
    log_func(f"右侧买点: {'启用' if enable_buy_points else '禁用'}")
    ...
    if stocks.empty:
        log_func("获取股票列表失败")
        return pd.DataFrame(), 0
    stocks = stocks[stocks['industry'].isin(selected_industries)].to_dict('records')
   
    prefixes = []
    if "600" in selected_boards:
        prefixes.extend(["sh.60", "sh.601", "sh.603", "sh.605"])
    if "300" in selected_boards:
        prefixes.append("sz.300")
    if "000" in selected_boards:
        prefixes.extend(["sz.000", "sz.001", "sz.002"])
    if "688" in selected_boards:
        prefixes.append("sh.688")
   
    stocks = [s for s in stocks if any(s['code'].startswith(p) for p in prefixes)]
    log_func(f"待扫描股票: {len(stocks)} 只")
   
    start_date = (datetime.now() - timedelta(days=400)).strftime('%Y-%m-%d')
    end_date = datetime.now().strftime('%Y-%m-%d')
   
    if not ensure_login() and DATA_SOURCE == "baostock":
        log_func("baostock登录失败，将尝试使用东方财富数据源")
   
    log_func("开始扫描...")
    results = []
    stats = {'total': len(stocks), 'qualified': 0, 'reasons': {}}
    ...
    def process(stock):
        ...
        try:
            ...
            if df is None or len(df) < 30:
                if debug_mode:
                    log_func(f"过滤 {code} {name}: 数据不足")
                return None, "数据不足"
           
            df = calc_indicators(df, all_ma_periods)
            last = df.iloc[-1]
            prev = df.iloc[-2] if len(df) >= 2 else None
           
            if 'ST' in name:
                if debug_mode:
                    log_func(f"过滤 {code} {name}: ST股票")
                return None, "ST股票"
            ...
            for i in range(-3, 0):
                if i >= len(df):
                    continue
                d = df.iloc[i]
                open_val = d.get('open', 0)
                high_val = d.get('high', 0)
                low_val = d.get('low', 0)
                close_val = d.get('close', 0)
                if open_val == high_val == low_val == close_val:
                    vol = d.get('volume', 0)
                    prev_vol = df.iloc[i-1].get('volume', 1)
                    if prev_vol > 0 and vol < prev_vol * 0.2:
                        if debug_mode:
                            log_func(f"过滤 {code} {name}: 一字板无量")
                        return None, "一字板无量"
           
            # 黄牛信号
            if enable_huangniu:
                huangniu_signal = last.get('HUANGNIU_SIGNAL', 0)
                if huangniu_signal == 1:
                    if debug_mode:
                        log_func(f"过滤 {code} {name}: 黄牛卖出信号")
                    return None, "黄牛卖出信号"
                elif huangniu_signal == 0:
                    if debug_mode:
                        log_func(f"过滤 {code} {name}: 无黄牛信号")
                    return None, "无黄牛信号"
           
            # 涨停检查
            if enable_limit_strength:
                has_limit = last.get('HAS_LIMIT', False)
                limit_days_ago = last.get('LIMIT_DAYS_AGO', 999)
                if not has_limit or limit_days_ago > limit_days_max:
                    if debug_mode:
                        log_func(f"过滤 {code} {name}: 无{limit_days_max}天内涨停")
                    return None, f"无{limit_days_max}天内涨停"
            ...
            if turn < min_turn or turn > max_turn:
                if debug_mode:
                    log_func(f"过滤 {code} {name}: 换手{turn:.1f}%")
                return None, f"换手{turn:.1f}%"
           
            # 均线偏离度
            ma_col = f'MA{ma_period}'
            if ma_col not in df.columns:
                if debug_mode:
                    log_func(f"过滤 {code} {name}: 无{ma_period}日线")
                return None, f"无{ma_period}日线"
            ma_value = last[ma_col]
            if pd.isna(ma_value):
                if debug_mode:
                    log_func(f"过滤 {code} {name}: {ma_period}日线无效")
                return None, f"{ma_period}日线无效"
            current_price = last['close']
            if pd.isna(current_price):
                current_price = 0
            deviation = (current_price - ma_value) / ma_value * 100
            if deviation < ma_deviation_lower or deviation > ma_deviation_upper:
                if debug_mode:
                    log_func(f"过滤 {code} {name}: 偏离{deviation:.1f}%")
                return None, f"偏离{deviation:.1f}%"
           
            # 高位判断
            year_high = df['high'].max()
            if pd.isna(year_high) or year_high == 0:
                year_high = current_price
            high_pos = current_price / year_high if year_high > 0 else 1
            if high_pos > high_pos_threshold:
                if debug_mode:
                    log_func(f"过滤 {code} {name}: 高位{high_pos*100:.0f}%")
                return None, f"高位{high_pos*100:.0f}%"
           
            # 我的盈利模式识别
            my_pattern_match = False
            t2_pct = 0
            t1_pct = 0
            repair_ratio = 0
            pattern_desc = ""
            
            if enable_my_pattern:
                match, t2, t1, repair, desc = check_my_profit_pattern(df, debug=False)
                if match:
                    my_pattern_match = True
                    t2_pct = t2
                    t1_pct = t1
                    repair_ratio = repair
                    pattern_desc = desc
                    ...
                    if debug_mode:
                        log_func(f"过滤 {code} {name}: {desc}")
                    return None, "不匹配形态"
           
            # 右侧买点计算
            upper, lower, is_prev_red = None, None, None
            if enable_buy_points and prev is not None:
                upper, lower, is_prev_red = get_buy_points(prev, last)
           
            result = {
                "名称": name, "代码": code, "当前价格": safe_round(current_price, 2),
                "涨跌幅%": safe_round(last.get('pctChg', 0), 2),
                "换手率%": safe_round(turn, 2),
                "参考均线": f"{ma_period}日", "均线值": safe_round(ma_value, 2),
                "偏离度%": safe_round(deviation, 2),
                "右侧买点(上)": upper if upper else "-", 
                "右侧买点(下)": lower if lower else "-",
                "所属行业": industry
            }
            
            # 添加我的模式信息
            if my_pattern_match:
                result["T-2跌幅%"] = safe_round(t2_pct, 2)
                result["T-1涨幅%"] = safe_round(t1_pct, 2)
                result["修复比"] = safe_round(repair_ratio, 2)
                result["形态描述"] = pattern_desc
            
            for p in [5, 10, 20, 30, 60]:
                ma = last.get(f'MA{p}')
                if ma and not pd.isna(ma):
                    result[f'{p}日均线'] = safe_round(ma, 2)
            return result, None
        except Exception as e:
            if debug_mode:
                log_func(f"过滤 {code} {name}: 错误-{str(e)[:50]}")
            return None, f"错误"
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for future in as_completed(futures):
            ...
            if result:
                ...
                stats['qualified'] += 1
                log_func(f"✓ {result['名称']}({result['代码']})")
            elif reason and "取消" not in reason and debug_mode:
                stats['reasons'][reason] = stats['reasons'].get(reason, 0) + 1
            
            if completed % 50 == 0:
                progress_func(int(completed / len(stocks) * 100))
                log_func(f"进度: {completed}/{len(stocks)} ({int(completed/len(stocks)*100)}%) - 已发现{stats['qualified']}只")
   
    progress_func(100)
    log_func(f"\n扫描完成: 共{len(stocks)}只, 选出{len(results)}只")
    
    if debug_mode and stats['reasons']:
        log_func("过滤原因统计:")
        for r, c in sorted(stats['reasons'].items(), key=lambda x: -x[1])[:10]:
            log_func(f"  {r}: {c}只")
   
    return pd.DataFrame(results), len(stocks)

def export_to_excel(df, log_func):
    if df.empty:
        log_func("没有符合条件的股票")
        return None
    try:
        desktop = os.path.join(os.path.expanduser("~"), "Desktop")
        output = os.path.join(desktop, f"选股结果_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx")
...
def main(params, log_func, progress_func):
    global stop_scan_flag
    stop_scan_flag = False
    try:
        df, scanned = scan_stocks(**params)
        if not stop_scan_flag and not df.empty:
            output = export_to_excel(df, log_func)
            if output:
                log_func(f"\n结果已保存: {output}")
        log_func(f"\n扫描完成，共{scanned}只，选出{len(df)}只")
    except Exception as e:
        log_func(f"错误: {e}")
    progress_func(100)
